...
model SubTopic {
  ...
  index           Int                        // 在总知识点内的顺序
  title           String
  metadata        Json                       // 子知识点元数据（summary/keywords/relatedPages）
  createdAt       DateTime  @default(now())
...
采用"先宏观后微观"的分层提取策略，避免批次边界切断知识点：

1. **第一轮：提取总知识点结构**
   - 读取整个 PDF 的目录/标题/章节结构
   - 识别所有 TopicGroup（总知识点）的标题和页码范围
   - 不读取具体内容，仅识别结构骨架

2. **第二轮：逐个提取子知识点**
   - 按 TopicGroup 的页码范围，分别读取每个总知识点的内容
   - 在该范围内提取 SubTopic（子知识点）
   - 确保子知识点不会跨越总知识点边界

3. **分批处理**
   - 每批最多处理 120 页，避免超出 AI 输入限制
   - 批次边界按 TopicGroup 边界对齐，不会切断知识点
...
- **不预提取**：避免增加结构提取时间和成本
- **讲解时按需**：当 AI 生成讲解时，按以下条件调用 Mathpix

#### 触发条件

1. **AI 判断触发**：AI 在生成讲解的 `mathematics` 部分时，判断是否需要引用 PDF 中的公式图片
2. **具体流程**：
   - AI 分析子知识点内容，识别需要讲解的公式
   - 如果 PDF 文本中已包含可用的 LaTeX，直接使用
   - 如果需要识别图片中的公式，调用 Mathpix API
3. **返回格式**：Mathpix 返回的 LaTeX 嵌入到讲解内容的 `mathematics` 字段中
...
- [ ] 测试通过判定：必须答完所有题，核心 ≥2/3 通过，支持 1/1 通过
- [ ] 单题跳过：同一题答错 3 次后可跳过
- [ ] 薄弱点标记：总知识点累计答错 ≥ 3 次
